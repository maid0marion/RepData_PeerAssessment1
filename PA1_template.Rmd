...
```{r hist_tot_steps, message=FALSE}
library(dplyr)
...
The results report the mean total number of steps taken daily is **`r avg_steps`** and the median is **`r med_steps`**.
...
The results determined that the 5-minute interval **`r max_interval`**, or between 8:35 and 8:40 AM, on average across all days in the dataset, had the maximum number of steps taken which was **`r max_steps`** steps.
...
The result of this calculation determined there are **`r cnt_na`** missing values in the dataset.
...
Using the new dataset, the results report the mean total number of steps taken daily is **`r new_avg_steps`** and the median is **`r new_med_steps`**.
...
```{r new_interval_avg, message=FALSE}
library(plyr)
